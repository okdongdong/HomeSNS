    ...
    <v-card>
      ...
      <v-card-text>
        <v-form class="px-3" ref="form">
          <v-text-field label="일정" v-model="event.title" prepend-icon="mdi-folder-marker" :rules="inputRules">
          </v-text-field>
          <v-textarea label="상세설명" v-model="event.content" prepend-icon="mdi-pencil" :rules="inputRules">
          </v-textarea>
          <v-row>
              ...
              <v-menu>
                <template v-slot:activator="{on}">
                  ...
                  <v-text-field slot="activator" label="시작일" readonly prepend-icon="mdi-calendar-month" v-on="on"
                                :value="event.startDate" class=""></v-text-field>
                </template>
            ...
            <v-col cols="6" class="pb-0">
              <v-menu :close-on-content-click="false" v-model="startTimer" offset-y>
                <template v-slot:activator="{ on }">
                  <v-text-field label="시작 시간" readonly :value="event.startTime" prepend-icon="mdi-timer" v-on="on">
                  </v-text-field>
                </template>
          ...
          <v-row>
              ...
              <v-menu>
                <template v-slot:activator="{on}">
                  <v-text-field slot="activator" label="종료일" readonly prepend-icon="mdi-calendar-month" v-on="on"
                                :value="event.endDate" class=""></v-text-field>
                </template>
            ...
            <v-col cols="6" class="pt-0">
              <v-menu :close-on-content-click="false" v-model="endTimer" offset-y>
                <template v-slot:activator="{ on }">
                  <v-text-field label="종료 시간" readonly :value="event.endTime" prepend-icon="mdi-timer" v-on="on">
                  </v-text-field>
                </template>
